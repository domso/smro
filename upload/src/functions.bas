...
Sub load_game(lvl As Integer)
	...
	For i As Integer= 1 To 2
		If Mid(global.world,1,1)<>"" And Mid(global.world,1,1)<>"0" Then
...
Sub update_game
		...
		For j As Integer = 1 To 1
			If global.item_belegt(i,j)<>0 then 
				If Abs(entityX(global.item(i,j))-EntityX(player(0).obj))<8 and Abs(entityy(global.item(i,j))-Entityy(player(0).obj))<8 And Abs(entityz(global.item(i,j))-Entityz(player(0).obj))<8 Then
					...
					player(0).modi+=1
					player(0).punkte+=player(0).meter_sec*player(0).modi
	...
	moveEntity player(0).obj,0,-0.5,-(player(0).max_speed-player(0).speed_minus)
	player(0).meter+=(player(0).max_speed-player(0).speed_minus)
	player(0).speed_minus+=0.01
	...
	If Entitypitch(player(0).obj)<>0 Then
  		If Entitypitch(player(0).obj)>0 Then turnentity player(0).obj,-1,0,0	
  		If Entitypitch(player(0).obj)<0 Then turnentity player(0).obj,1,0,0
   EndIf
   If EntityRoll(player(0).obj)<>0 Then
   	If EntityRoll(player(0).obj)>0 Then turnentity player(0).obj,0,0,-1
   	If EntityRoll(player(0).obj)<0 Then turnentity player(0).obj,0,0,1
   EndIf

	player(0).meter_sec=(player(0).meter-player(0).start_meter)/(Timer-player(0).start_zeit)\10000
