...
' IRC: IRC://MLN.ath.cx/#mln
'##############################################################################################################
' Free for NON-comercial use! For comercial usage send me a mail -> Admin[at]MLN.ath.cx or IRC and u get a free usage
'##############################################################################################################
...
Do
    ...
    Do Until TPtr = 0
        ...
        If TPtr->T_ThreadOn = 3 Then
            TID = TPtr->V_TSNEID
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= ThreadON 3"
            #ENDIF
            TPtr->T_ThreadOn = 4
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= ThreadON 4"
            #ENDIF
            TThPtr = TPtr->T_Thread
            TEvent = TPtr->V_Event
            MutexUnLock(TSNE_INT_Mutex)
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Unlock"
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Wait..."
            #ENDIF
            ThreadWait(TThPtr)
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Wait-K"
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Call-Dis..."
            #ENDIF
            #IF DEFINED(TSNE_SUBCALLBACK)
            ...
            #ENDIF
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Call-Dis-K"
            #ENDIF
            TSNE_INT_Del(TPtr)
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Lock..."
            #ENDIF
            MutexLock(TSNE_INT_Mutex)
            #IF DEFINED(_TSNE_DODEBUG_)
                Print "=[" & Str(TID) & "]=[TSNE]=[TMA]= Lock-K"
            #ENDIF
        End If
        TPtr = TNPtr
    Loop
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[TSNE]=[TMA]= M-Lock..."
    #ENDIF
    MutexLock(TSNE_INT_Mutex_Master)
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[TSNE]=[TMA]= M-Lock-K"
    #ENDIF
    ...
    MutexUnLock(TSNE_INT_Mutex_Master)
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[TSNE]=[TMA]= M-Unlock"
    #ENDIF
    MutexUnLock(TSNE_INT_Mutex)
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[TSNE]=[TMA]= Unlock"
    #ENDIF
    Sleep 1000, 1
Loop
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[TSNE]=[TMA]= END SUB"
#ENDIF
End Sub
...
'|End If
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[" & Str(V_TSNEID) & "]=[TSNE]=[DIS]= Lock..."
#ENDIF
MutexLock(TSNE_INT_Mutex)
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[" & Str(V_TSNEID) & "]=[TSNE]=[DIS]= Lock-K"
#ENDIF
Dim TPtr as TSNE_Socket Ptr = TSNE_INT_GetPtr(V_TSNEID)
If TPtr = 0 Then MutexUnLock(TSNE_INT_Mutex): Return TSNE_Const_TSNEIDnotFound
...
MutexUnLock(TSNE_INT_Mutex)
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[" & Str(V_TSNEID) & "]=[TSNE]=[DIS]= Unlock"
#ENDIF
close_(TSock)
Return TSNE_Const_NoError
End Function
...
'--------------------------------------------------------------------------------------------------------------
'Private Sub TSNE_DisconnectAll()
...
TSD->V_Event.V_AnyPtr = V_CallbackBackPtr
R_TSNEID = TSD->V_TSNEID
TSD->T_Thread = ThreadCreate(cast(Any Ptr, @TSNE_INT_Thread_Event), cast(Any Ptr, R_TSNEID), V_StackSizeOverride)
MutexUnLock(TSNE_INT_Mutex)
...
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[" & Str(Cast(UInteger, V_TSNEID)) & "]=[TSNE]=[EVT]= Lock..."
#ENDIF
MutexLock(TSNE_INT_Mutex)
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[" & Str(Cast(UInteger, V_TSNEID)) & "]=[TSNE]=[EVT]= Lock-K"
#ENDIF
...
'|              If select_(TSock + 1, @TFDSet, 0, 0, @TTV) = -1 Then Exit Do
        ...
        Case TSNE_P_TCP
            If TSD->V_IsServer <> 1 Then
                MutexUnLock(TSNE_INT_Mutex)
                #IF DEFINED(_TSNE_DODEBUG_)
                    Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Unlocked"
                #ENDIF
                #IF DEFINED(TSNE_SUBCALLBACK)
...
'|                  If select_(TSock + 1, @TFDSet, 0, 0, @TTV) = -1 Then Exit Do
                    ...
                    If (FD_ISSET(TSock, @TFDSet)) <> 0 Then
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Event"
                        #ENDIF
                        If TSock = INVALID_SOCKET Then Exit Do
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Dat"
                        #ENDIF
                        ...
                        MemCpy(StrPtr(T), StrPtr(TBuffer), TLenB)
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Dat Lock..."
                        #ENDIF
                        MutexLock(TSNE_INT_Mutex)
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Dat Lock-K"
                        #ENDIF
                        TSD = TSNE_INT_GetPtr(TTSNEID): If TSD = 0 Then MutexUnLock(TSNE_INT_Mutex): Exit Do
                        TSD->T_DataIn += TLenB
                        MutexUnLock(TSNE_INT_Mutex)
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Dat Unlock"
                        #ENDIF
                        T = Mid(T, 1, Len(T) - 1)
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Dat Call... (" & Str(Len(T)) & ")"
                        #ENDIF
                        #IF DEFINED(TSNE_SUBCALLBACK)
                        ...
                        #ENDIF
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-C]= Dat Call-K"
                        #ENDIF
                    End If
                Loop
            Else
                MutexUnLock(TSNE_INT_Mutex)
                #IF DEFINED(_TSNE_DODEBUG_)
                    Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-S]= Unlocked"
                #ENDIF
...
'|                  If selectsocket(TSock + 1, @TFDSet, 0, 0, @TTV) = SOCKET_ERROR Then
                    ...
                    If (FD_ISSET(TSock, @TFDSet)) Then
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-S]=  ACP..."
                        #ENDIF
                        TNSock = accept(TSock, 0, 0)
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-S]=  ACP:" & Str(TNSock)
                        #ENDIF
                        ...
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-S]= BWL Lock..."
                        #ENDIF
                        MutexLock(TSNE_INT_Mutex)
                        #IF DEFINED(_TSNE_DODEBUG_)
                        ...
                        End Select
                        #IF DEFINED(_TSNE_DODEBUG_)
                            Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]=[TCP-S]= BWL Unlock"
                        #ENDIF
                    End If
                Loop
            End If
    ...
    End Select
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]= Exit Lock"
    #ENDIF
    MutexLock(TSNE_INT_Mutex)
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]= Exit Lock-K"
    #ENDIF
End If
...
If TSD <> 0 Then
    ...
    TSD->T_ThreadOn = 3
    #IF DEFINED(_TSNE_DODEBUG_)
        Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]= Exit ThreadON 3"
    #ENDIF
End If
MutexUnLock(TSNE_INT_Mutex)
#IF DEFINED(_TSNE_DODEBUG_)
    Print "=[" & Str(TTSNEID) & "]=[TSNE]=[EVT]= Exit Unlocked"
#ENDIF
End Sub
