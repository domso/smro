...
Sub global_type.quit(temp_mx As Integer,temp_my As Integer,temp_mb As Integer)
	...
	If temp_mx>temp_x+11 And temp_mx<temp_x+46 And temp_my>temp_y+11 And temp_my<temp_y+46 And temp_mb=1 Then
		...
		If this.configValue.config(4).value=1 Then Shell "kill pidof(server)"
...
Sub game_type.start
	...
	'Shell "LD_LIBRARY_PATH=. ./main "+global.serverAddresse(global.configvalue.config(6).value)+" "+Str(global.serverPort(global.configvalue.config(6).value))+" "+send_game_string+" "+Str(this.daytime)+ConfigParam
